...
struct VOXELM
{
  ...
  int group;			//group No.
  int new_group;
  struct VOXELM<T> *head;		//pointer to the first voxel in the group in the linked list
  struct VOXELM<T> *last;		//pointer to the last voxel in the group
  struct VOXELM<T> *next;		//pointer to the next voxel in the group
};
...
struct EDGE
{
  T reliab;			//reliabilty of the edge and it depends on the two voxels
  VOXELM<T> *pointer_1;		//pointer to the first voxel
  VOXELM<T> *pointer_2;		//pointer to the second voxel
...
template <typename T> 
void  initialiseVOXELs(T *WrappedVolume, unsigned char *input_mask, unsigned char *extended_mask, VOXELM<T> *voxel, int volume_width, int volume_height, int volume_depth)
{
  VOXELM<T> *voxel_pointer = voxel;
  T *wrapped_volume_pointer = WrappedVolume;
...
template <typename T> 
T wrap(T voxel_value)
{
  T wrapped_voxel_value;
...
template <typename T> 
int find_wrap(T voxelL_value, T voxelR_value)
{
  T difference;
  int wrap_value;
...
template <typename T> 
void extend_mask(unsigned char *input_mask, unsigned char *extended_mask, int volume_width, int volume_height, int volume_depth, params_t<T> *params)
{
  int n, i, j;
  int vw = volume_width;
...
template <typename T>
void calculate_reliability(T *wrappedVolume, VOXELM<T> *voxel, int volume_width, int volume_height, int volume_depth, params_t<T> *params)
{
  int frame_size  = volume_width * volume_height;
  int volume_size = volume_width * volume_height * volume_depth;
  VOXELM<T> *voxel_pointer;
  T H, V, N, D1, D2, D3, D4, D5, D6, D7, D8, D9, D10;
  T *WVP;
  int n, i, j;
...
//next neighbour
template <typename T>
void  horizontalEDGEs(VOXELM<T> *voxel, EDGE<T> *edge, int volume_width, int volume_height, int volume_depth, params_t<T> *params)
{
  int n, i, j;
...
template <typename T>
void  verticalEDGEs(VOXELM<T> *voxel, EDGE<T> *edge, int volume_width, int volume_height, int volume_depth, params_t<T> *params)
{
  int n, i, j;
  int no_of_edges = params->no_of_edges;
  VOXELM<T> *voxel_pointer = voxel;
...
template <typename T>
void  normalEDGEs(VOXELM<T> *voxel, EDGE<T> *edge, int volume_width, int volume_height, int volume_depth, params_t<T> *params)
{
  int n, i, j;
  int no_of_edges = params->no_of_edges;
  int frame_size = volume_width * volume_height;
...
template <typename T>
void gatherVOXELs(EDGE<T> *edge, params_t<T> *params)
{
  int k;
  VOXELM<T> *VOXEL1;
  VOXELM<T> *VOXEL2;
...
//unwrap the volume
template <typename T>
void unwrapVolume(VOXELM<T> *voxel, int volume_width, int volume_height, int volume_depth)
{
  int i;
  int volume_size = volume_width * volume_height * volume_depth;
  VOXELM<T> *voxel_pointer=voxel;
  ...
  for (i = 0; i < volume_size; i++)
    {
      voxel_pointer->value += TWOPI * (T)(voxel_pointer->increment);
      voxel_pointer++;
    }
}
...
template <typename T>
void  maskVolume(VOXELM<T> *voxel, unsigned char *input_mask, int volume_width, int volume_height, int volume_depth)
{
  VOXELM<T> *pointer_voxel = voxel;
...
//phase map.
template <typename T>
void  returnVolume(VOXELM<T> *voxel, T *unwrappedVolume, int volume_width, int volume_height, int volume_depth)
{
  int i;
  int volume_size = volume_width * volume_height * volume_depth;
  T *unwrappedVolume_pointer = unwrappedVolume;
  VOXELM<T> *voxel_pointer = voxel;
...
template <typename T>
void unwrap3D(T* wrapped_volume, T* unwrapped_volume, unsigned char* input_mask,
	 int volume_width, int volume_height, int volume_depth,
	 int wrap_around_x, int wrap_around_y, int wrap_around_z)
{
  params_t<T> params = {TWOPI, wrap_around_x, wrap_around_y, wrap_around_z, 0};
  ...
  voxel = (VOXELM<T> *) calloc(volume_size, sizeof(VOXELM<T>));
  ...
  if (params.no_of_edges > 0) //avoid crashing MB
    {
...
void mexFunction(int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
    ...
    unsigned char* input_mask = (unsigned char*)mxCalloc(nx*ny*nz,sizeof(unsigned char));
    ...
    switch(mxGetClassID(prhs[0]))
    {
        case mxSINGLE_CLASS: plhs[0] = mxCreateNumericArray(ndims, dims, mxSINGLE_CLASS, mxREAL);
                             unwrap3D((float*)mxGetData(prhs[0]), (float*)mxGetData(plhs[0]),
                             input_mask, nx, ny, nz, wrap_around_x, wrap_around_y, wrap_around_z);
                             break;

        case mxDOUBLE_CLASS: plhs[0] = mxCreateNumericArray(ndims, dims, mxDOUBLE_CLASS, mxREAL);
                             unwrap3D((double*)mxGetData(prhs[0]), (double*)mxGetData(plhs[0]),
                             input_mask, nx, ny, nz, wrap_around_x, wrap_around_y, wrap_around_z);
                             break;
