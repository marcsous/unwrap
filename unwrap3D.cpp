...
void mexFunction(int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
    ...
    if(ndims != 3)
        mxShowCriticalErrorMessage("argument 1 must be a 3D volume");
    ...
    unsigned char* input_mask = (unsigned char*) calloc(nx*ny*nz,sizeof(unsigned char));
    
    if (nrhs==2)
    {
        if(mxGetNumberOfDimensions(prhs[1]) != 3)
            mxShowCriticalErrorMessage("argument 2 must be a 3D volume");
        ...
        {
            case mxLOGICAL_CLASS: for (int i = 0; i<nx*ny*nz; i++) input_mask[i] = ((bool*)mxGetData(prhs[1]))[i] == 0;
            break;
            
            case mxCHAR_CLASS:
            case mxINT8_CLASS:
            case mxUINT8_CLASS: for (int i = 0; i<nx*ny*nz; i++) input_mask[i] = ((int8_t*)mxGetData(prhs[1]))[i] == 0;
            break;
            
            case mxINT16_CLASS:
            case mxUINT16_CLASS: for (int i = 0; i<nx*ny*nz; i++) input_mask[i] = ((int16_t*)mxGetData(prhs[1]))[i] == 0;
            break;
            
            case mxINT32_CLASS:
            case mxUINT32_CLASS:
            case mxSINGLE_CLASS: for (int i = 0; i<nx*ny*nz; i++) input_mask[i] = ((int32_t*)mxGetData(prhs[1]))[i] == 0;
            break;
            
            case mxINT64_CLASS:
            case mxUINT64_CLASS:
            case mxDOUBLE_CLASS: for (int i = 0; i<nx*ny*nz; i++) input_mask[i] = ((int64_t*)mxGetData(prhs[1]))[i] == 0;
            break;
        ...
        // check mask
        int nnz = 0;
        for (int i = 0; i<nx*ny*nz; i++) nnz += input_mask[i] == 0;
    ...
    float* wrapped_volume = (float*)mxGetData(prhs[0]);
    float* unwrapped_volume = (float*)mxGetData(plhs[0]);
    int volume_width = nx;
    int volume_height = ny;
    int volume_depth = nz;
    int wrap_around_x = 0; // circular wrap in x
    int wrap_around_y = 0; // circular wrap in y
    int wrap_around_z = 0; // circular wrap in z
    
    unwrap3D(wrapped_volume, unwrapped_volume, input_mask,
